...
fit_conf <- function(simdat, empdat, w) {
    
    guess <- c(1/(0.15 + w), 1.5/(0.15 + w), 2/(0.15 + w))
    
    params <- list("w" = w,
                   "m1_correct" =   filter(simdat, choice == 1)$m1,
...
weight <- seq(0, 1, 0.1)
fit <- foreach(w = weight, .combine = "rbind", .packages = c("magrittr", "dplyr")) %dopar% {
    f <- fit_conf(simdat = simdat, empdat = df01, w = w)
    f$w <- w
    print(f)
}
fit
