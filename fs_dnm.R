...
dnm <- ggplot(acc) + geom_point(aes(x = val3, y = Accuracy, color = subj)) +
    geom_line(mapping = aes(x = val3, y = pred, color = subj)) + ylim(0.49, 0.9) + xlab("Dud stimulus") + ggtitle("dnm")  + guides(color = guide_legend(title = NULL))
dnm
...
ggplot(conf_dat) + geom_line(aes(x = condition, y = mean_be, color = subj)) + 
    ...
    facet_grid(. ~ max) + ggtitle("dnm") -> dnm_be
dnm_be
...
ggplot(conf_dat) + geom_line(aes(x = condition, y = mean_be2, color = subj)) + 
    ...
    facet_grid(. ~ max) + ggtitle("dnm") -> dnm_be2
dnm_be2
...
ggplot(conf_dat) + geom_line(aes(x = condition, y = mean_dce, color = subj)) + 
    ...
    facet_grid(. ~ max) + ggtitle("dnm") -> dnm_dce
dnm_dce